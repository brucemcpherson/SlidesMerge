/**
 * This uses muicss material design and  is a way of dynamically generating 
 * a multipage settings menu, initially for use with add-ons, bur can be used for any thing
 * It contains a bunch of templates for different element types
 * and maintains different states for element values, each of which has a unique name, and is independent of layout.
 * it needs a detail (which elements are needed), and a layout (how to organize them) object to define the pages
 * the idea is that each page has an apply buton and a back button
 * navigating away from the settings tab has any changes in that settings page activated
 * so you can see the effect of the changes, back should restore to the enter status
 * and applying should mak the changes permanent.
 * each template item has this
       on: {
        change: self.defaultOnChange
      }
 * that function is able to detect which conteol element the event belongs to
 * and to deduce its apply and back button
 * it takes care of enabling the apply button if a change is detected
 * if other behavior is needed, this can be overridden when the element is defined using the template
 *
 * there are 3 values states getCurrent(), getInitial(), getStandard() - there is an applySettings_(object), 
 * and restoreStandard() etc... to manage all of this
 * - current what the current state of the resetable elements are (elements can be resetable/non resetable to appear in this list or not)
 * - initial what they were when set up (for example if they had been saved and restored to superceded the defaults)
 * - standard the default values for the app
 *
 * getElements() return the elements in the structure
 * getElements().controls.elementName returns the html control thats been created for a particular item
 * @constructor Elementer
 */
function Elementer() {
  ...
  /**
   * can be used to detect the apply button for the current branch
   * and set it to enable apply when an element changes
   * @param {Elementer} the elementer
   * @param {string} branch the branch
   * @param {object} ob the object being changed
   * @param {object} e the event args
   * @return {Elementer} the elementer
   */
  self.defaultOnChange = function  (elementer , branch, ob , e) {
    
    var controls = elementer.getElements().controls;
    var page = elementer.getLayout().pages[branch];
    var resetButton = (page ? (page.items || []) : [])
    .map(function(d) {
      return controls[d] && controls[d].type === "button" ? controls[d] : null;
    })
    .reverse()
    .filter (function(d) {
      return d;
  ...
  */
  ...
  self.applySettings = function (target) {
    ...
    Object.keys(target).forEach(function(k) {
      if (elements_.values[k]) {
        elements_.controls[k][elements_.values[k].property] = target[k];
      }
      else {
        console.log("warning-legacy property ",k,"ignored");
      }
    });
  ...
  self.getCurrent = function () {
    return Object.keys(initial_).reduce(function(p,c) {
      if (!elements_.values.hasOwnProperty(c)) {
        console.log("warning-legacy property ",c,"ignored");
      }
      else {
        ...
        if (layout.pages[d]) {
          ...
          if (ob.tag) {
            ...
            if (elem && ob.values && ob.values.property) {
              ...
              // only resetable values are stored
              // since this is used for saving properties
